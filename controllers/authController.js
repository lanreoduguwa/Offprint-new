...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:700px){.stats{grid-template-columns:1fr 1fr}.form-grid{grid-template-columns:1fr}.wrapper{padding:16px}}
</style>
</head>
<body>
<header class="topbar">
...
<div class="wrapper">
  <div class="stats">
    <div class="stat-card"><div class="label">Total Items</div><div class="value" id="stat-total">—</div></div>
  ...
  <div class="panel">
    ...
    <div class="panel-body">
      <form id="add-form" enctype="multipart/form-data">
        <div class="form-grid">
          <div class="fg"><label>Title *</label><input type="text" name="title" placeholder="e.g. Corporate Branding for ABC Ltd" required></div>
        ...
        <div class="form-grid full" style="margin-top:14px">
          <div class="fg"><label>Description</label><textarea name="description" placeholder="Brief description (optional)"></textarea></div>
        </div>
        <div class="form-grid" style="margin-top:14px">
          <div class="fg"><label>Image *</label><input type="file" name="image" accept="image/*" id="img-input" required><img id="img-preview" alt="Preview"></div>
          <div class="fg" style="justify-content:center">
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;margin-top:28px">
              <input type="checkbox" name="featured" value="true" style="width:18px;height:18px;accent-color:var(--gold)"> Mark as Featured ⭐
            </label>
          </div>
        </div>
  ...
  <div class="panel">
    ...
    <div class="panel-body"><div id="portfolio-list"><div class="spinner"></div></div></div>
  </div>
</div>
<div class="modal-bg" id="edit-modal">
  <div class="modal">
    ...
    <form id="edit-form">
      ...
      <div class="form-grid" style="margin-top:14px">
        <div class="fg"><label>Replace Image (optional)</label><input type="file" id="edit-image" accept="image/*">
        ...
        <div class="fg" style="justify-content:center">
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer;margin-top:28px">
            <input type="checkbox" id="edit-featured" style="width:18px;height:18px;accent-color:var(--gold)"> Mark as Featured ⭐
          </label>
        </div>
      </div>
      <div class="form-actions">
...
<script>
  ...
  async function loadPortfolio(){
    ...
    try{
      ...
      if(!data.success){list.innerHTML='<div class="empty">Failed to load items.</div>';return}
      ...
      if(!data.data.length){list.innerHTML='<div class="empty"><p>No portfolio items yet.</p></div>';return}
      list.innerHTML='<div class="portfolio-grid">'+data.data.map(item=>\`
        <div class="p-card">
          ...
          <div class="p-card-body">
            ...
            <div class="actions">
              <button class="btn btn-outline btn-sm" onclick='openEditModal(\${JSON.stringify(item)})'>Edit</button>
              <button class="btn btn-danger btn-sm" onclick="deleteItem('\${item._id}','\${item.title.replace(/'/g,&quot;&quot;)}')">Delete</button>
            </div>
          </div>
        </div>
      \`).join('')+'</div>';
  ...
  document.getElementById("img-input").addEventListener("change",function(){
    ...
    const p=document.getElementById("img-preview");p.src=URL.createObjectURL(f);p.style.display="block";
  });
  function resetPreview(){const p=document.getElementById("img-preview");p.style.display="none";p.src=""}
  document.getElementById("add-form").addEventListener("submit",async(e)=>{
    e.preventDefault();
    const btn=document.getElementById("add-btn"),status=document.getElementById("add-status"),fd=new FormData(e.target);
  ...
  async function deleteItem(id,title){
    ...
    try{
      ...
      const data=await res.json();
      if(data.success){toast("🗑️ Deleted.");loadPortfolio()}else toast("Failed: "+data.error,"error");
  ...
  document.getElementById("edit-image").addEventListener("change",function(){
    const f=this.files[0];if(!f)return;
    const p=document.getElementById("edit-img-preview");p.src=URL.createObjectURL(f);p.style.display="block";
  });
  document.getElementById("edit-form").addEventListener("submit",async(e)=>{
    e.preventDefault();
    const id=document.getElementById("edit-id").value,status=document.getElementById("edit-status"),fd=new FormData();
    ...
    const img=document.getElementById("edit-image").files[0];if(img)fd.append("image",img);
